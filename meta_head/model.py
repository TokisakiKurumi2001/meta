...
class MetaLlamaForCausalLM(LlamaPreTrainedModel):
    _tied_weights_keys = ["lm_head.weight"]

    def __init__(self, config):
        super().__init__(config)
        self.model = LlamaModel(config)
        self.model.set_input_embeddings(nn.Identity()) # empty the weight of old embedding layers
        self.vocab_size = config.vocab_size
        self.meta_head = nn.Identity()
    ...
    def save_meta_head(self, path):
        self.meta_head.save_train_weight_only(path)
    ...
    def forward(
        ...
        input_embeds = self.meta_head(input_ids, "input")
        ...
        logits = self.meta_head(hidden_states, "output")
        logits = logits.float()
