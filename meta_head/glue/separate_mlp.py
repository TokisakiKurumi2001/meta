...
class SeparateMLPConfig(PretrainedConfig):
...
class SeparateMLPPretrainedModel(PreTrainedModel):
    config_class = SeparateMLPConfig
    base_model_prefix = "separate_mlp"
...
class SeparateMLP(SeparateMLPPretrainedModel):
    def __init__(self, config: SeparateMLPConfig):
        ...
        self.w1_t = nn.Parameter(torch.normal(0, 0.02, (config.hidden_dim, config.input_dim)))
        self.w2_t = nn.Parameter(torch.normal(0, 0.02, (config.output_dim, config.hidden_dim)))
